<?php
namespace CirculoDeCredito\VerificacionExpediente\Interceptor;
...
use VerificacionExpedienteClientPhp\Client\Model\ErroresResponse;
use VerificacionExpedienteClientPhp\Client\Model\ErrorResponse;

class MiddlewareEvents
{
    
    function __construct(KeyHandler $signer)
    {
        $this->signer = $signer;
...
function build_error($code, $message){
    ...
    $errors = new ErroresResponse();
    $errors->setErrores(array($error));
